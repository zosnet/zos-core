...
namespace fc {
  ...
  class time_point_sec
  {
    public:
        ...
        //static time_point_sec maximum() { return time_point_sec(0xEEA4E9281); }
        static time_point_sec maximum() { return time_point_sec(0xffffffff); }
        static time_point_sec min() { return time_point_sec(0); }
        //static time_point_sec max() { return time_point_sec(0xEEA4E9280); }
        static time_point_sec max() { return time_point_sec(0xffffffff-1); }
        ...
        uint32_t get_date(const time_point_sec &a) const; 
        fc::string to_non_delimited_iso_string() const;
        fc::string to_db_string() const;
  ...
  inline uint32_t get_month_flag(const time_point_sec &tsrc)
  {
     time_t t = tsrc.sec_since_epoch();
     tm tmthis = *gmtime(&t);     
     return (tmthis.tm_mon + 1) + (tmthis.tm_year + 1900) *100;
  }
  inline uint32_t get_week_flag(const time_point_sec &tsrc)
  {  
     time_t t = tsrc.sec_since_epoch();
     tm tmthis = *gmtime(&t);
     tmthis.tm_year += 1900;
     t = mktime(&tmthis);
     tmthis = *gmtime(&t);
     return tmthis.tm_yday / 7 + 1;
  }  
   
  inline uint64_t get_align_time(const time_point_sec &tsrc)
  {
     return tsrc.sec_since_epoch() / 3600 * 3600; 
  }
  inline uint32_t get_hour_flag(const time_point_sec &tsrc)
  {
     return tsrc.sec_since_epoch() / 3600; 
  }
  inline uint32_t get_day_flag(const time_point_sec &tsrc)
  {
     time_t t = tsrc.sec_since_epoch();
     tm tmthis = *gmtime(&t);     
     return tmthis.tm_yday;
  } 
    
  inline time_point_sec add_month(const time_point_sec &tsrc,int imon,int iday = 0)
  {
	   time_t t = tsrc.sec_since_epoch();
     tm tmthis = *gmtime(&t);
     tm tmsrc = tmthis;
     tmthis.tm_mon += imon;
     while(tmthis.tm_mon>=12)
     {
        tmthis.tm_mon -= 12;
        tmthis.tm_year++;
     }
     tmthis.tm_sec  = 0;
     tmthis.tm_min  = 0;
     if(iday>0) tmthis.tm_mday = iday;
     time_t t1 = fc::make_utc_time(&tmthis);
     tm tmthis1 = *gmtime(&t1); 
     while(tmthis1.tm_year *12 + tmthis1.tm_mon > tmsrc.tm_year *12 + tmsrc.tm_mon + imon && t1 >0) 
     {
       t1 -= 24 * 3600;
       tmthis1 = *gmtime(&t1);
     }
     return fc::time_point_sec(fc::make_utc_time(&tmthis1));
  }
}
