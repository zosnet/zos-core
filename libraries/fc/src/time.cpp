...
namespace fc {
  ...
  fc::string time_point_sec::to_db_string()const
  {
    fc::string time_s = to_iso_string();
    time_s[10] = ' ';    
    return time_s;
  }
  ...
  uint32_t time_point_sec::get_date(const time_point_sec &a) const
  {
    if(a.utc_seconds <= utc_seconds)
      return 0;
    return (a.utc_seconds - utc_seconds -1) / (24 * 3600) + 1;
  }
